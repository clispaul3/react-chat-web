...
export class Group{
	...
	static getGroupInfo(targetId){
		return new Promise((resolve,reject)=>{
			axios.get('/weinong' + '/v1/group/query/' + targetId,{
  ...
  static deleteGroup(params,callback){
      axios.delete('/weinong' + '/v1/group/' + params.id + '?token=' + Ajax.user_token)
        .then(res=>{
              if(res.data.status=='1'){
                  callback(res.data)
	...
	static joinGroup(params, callback){
		...
		axios.post('/weinong' + '/v1/group/join',{
			id,
			token:Ajax.user_token,
			friends
		}).then(res=>{
	...
	static quitGroup(params, callback){
		const { id, friends } = params
		axios.put('/weinong' + '/v1/group/quit',{
			token:Ajax.user_token,
			id ,
			friends,
		}).then(res=>{
